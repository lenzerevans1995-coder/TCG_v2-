...
namespace TcgEngine.UI
{
    ...
    public class CardSelector : SelectorPanel
    {
        ...
        private void UpdateControls()
        {
            ...
            //Mouse scroll
            mouse_scroll += -MouseInput.GetMouseScroll();
            ...
            if (iability == null && MouseInput.IsRightClick() && timer > 1f)
                Hide();
        }
        ...
        private void UpdateCards()
        {
            ...
            //CCG-EDIT: mirror the selected card's rules text at panel-text size —
            //even a large card renders its desc too small to read comfortably
            if (iability == null && subtitle != null && selection_index >= 0 && selection_index < card_list.Count)
            {
                Card scard = card_list[selection_index];
                subtitle.text = scard != null ? scard.CardData.GetText() : "";
            }
        }
        ...
        {
            Vector2 localpoint;
            RectTransformUtility.ScreenPointToLocalPointInRectangle(content, MouseInput.GetMousePosition(), GetComponentInParent<Canvas>().worldCamera, out localpoint);
            return localpoint;
        }
