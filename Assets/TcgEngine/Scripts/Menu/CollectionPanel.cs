...
namespace TcgEngine.UI
{
    ...
    public class CollectionPanel : UIPanel
    {
        ...
        {
            UserData udata = Authenticator.Get().UserData;
            //CCG-EDIT: only true first-launch (no cards AND no decks) triggers onboarding;
            //kit's rewards.Length check re-showed the panel forever on offline saves
            if (udata != null && udata.cards.Length == 0 && udata.decks.Length == 0)
            {
